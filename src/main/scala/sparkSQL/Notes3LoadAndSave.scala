...
//par défaut si on ne spécifie pas le format de fichier source spark va considérer la source sous format parquet
  ...
  df.printSchema()
 // df.select("name","favorite_color").write.save("files/result_sous_users_parquet") on a importer les fonction lit et concat depuis org.apache.spark.sql.functions
