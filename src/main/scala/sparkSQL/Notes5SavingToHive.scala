...
object Notes5SavingToHive extends App {
  //pour travailler spark avec hive il faut appeler la méthode enableHiveSupport pour permettre spark à lire et écrire depuis le metastore de hive
  //spark n'a pas besoin que hive soit installé si on veut tester les traitements sur hive en locale car spark
  //va créer un métastore(base de donnée relationnelle) de type DERBY en local et il va stocker les données dans le système de fichier local
 ...
 val df = sparkss.read.json("files/example_1.json")
 // sparkss.sql("drop table if exists table1")
  df.write.mode(SaveMode.Overwrite).saveAsTable("table1")
 ...
 //création d'une table partitionnée
  val dfjson = sparkss.read.json("files/ismahan.json")
 dfjson.write.mode(SaveMode.Overwrite).partitionBy("ville").saveAsTable("partitionnedtable1")
}
