...
object Notes4SaveMode extends App{
  ...
  df.select("name","favorite_color").write.mode(SaveMode.Overwrite).save("files/result_sous_users_parquet")
  //2)si on veut ajouter les nouvelles données(fichiers) au encients fichiers existent déja  au données existantes on utilise Append
 ...
 df1.write.mode(SaveMode.Append).save("files/result_sous_users_parquet")
  //3)Ignore:c'est le meme principe du clause create table if not exists en sql c'à d si les données existent spark va rien faire
