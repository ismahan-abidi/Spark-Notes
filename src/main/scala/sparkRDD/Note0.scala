...
object Note0 extends App {
  ...
  //création d'une rdd mais pas l'éxécutée
  ...
  //si on va utiliser l'RDD X une autre fois il est préférable de la mettre dans la mémoire ou dans le disque
  x.foreach(println)
  x.persist()
  val y = x.reduce((a, b) => a + b) // y est un entier 65 dans cet exemple donc c'est une action
  println(y)
  //on peut aussi creér une rdd qui permet de lire tous les fichiers d'extention .txt comme suit:
  // val rdd2 = ismahen_sc.textFile("file:///home/abidi/*.txt") ou bien *.gz
 val rddfiles = ismahen_sc.wholeTextFiles("files")
 // rddfiles est une rdd des pairs sous forme clé valeur car la méthode wholeTextFiles retourne un couple de RDD
  // ou le clé est le path d'un fichier et la valeur est son contenu
  //x est un couple car wholeTextFile contient un couple( nom fichier , contenu fichier)
 rddfiles.foreach(x=>println("path fichier "+x._1+" contenue fichier "+x._2))
