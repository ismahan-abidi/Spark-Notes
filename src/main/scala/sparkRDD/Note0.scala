...
object Note0 extends App {
  ...
  //création d'une rdd mais pas l'éxécuté
  ...
  //si on va utiliser l'RDD X une autre fois il est préférable de la mettre dans la mémoire ou disque
  x.foreach(println)
  x.persist()
  val y = x.reduce((a, b) => a + b) // y est un entier dans cet exemple donc c'est une action
  println(y)
  //on peut ausssi creér une rdd qui permet de lire tous les fichiers d'extention .txt comme suit:
